...
export default function AnalysisClient({ enzymeId, initialFocus = null, initialStructureId = "" }: AnalysisClientProps) {
  ...
  function downloadMutationRecommendationCsv() {
    if (recommendationCandidates.length === 0) {
      return;
    }
    const blob = new Blob([buildMutationRecommendationCsv(recommendationCandidates)], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `mutation-recommendations-${enzymeId}.csv`;
    link.click();
    URL.revokeObjectURL(url);
  }
        ...
        <div className="border-b border-slate-200 px-4 py-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <button
              ...
              onClick={downloadMutationRecommendationCsv}
              type="button"
            >
              Download CSV
            </button>
